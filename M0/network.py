...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.conv_rdb8x0 = conv_layer(64, 64, kernel_size=3, groups=1, bias=True, negative_slope=1, bn=False, init_type='kaiming', fan_type='fan_in', activation=False, pixelshuffle_init=False, upscale=False, num_classes=False, weight_normalization=True)
        ...
        self.conv_rdb8x3 = conv_layer(int(64 * 2), 64, kernel_size=3, groups=1, bias=True, negative_slope=1, bn=False, init_type='kaiming', fan_type='fan_in', activation=False, pixelshuffle_init=False, upscale=False, num_classes=False, weight_normalization=True)
    ...
    def forward(self, low):
        ...
        # 64x branch starts
        low64x_beforeRDB = self.conv64x(self.downshuffle(low2x, 4))
        rdb1 = self.RDB1(low64x_beforeRDB)
        rdb2 = self.RDB2(rdb1)
        rdb8x0 = torch.cat((rdb1, rdb2), dim=1)
        rdb8x0 = self.rdball3(rdb8x0) + low64x_beforeRDB
        # rdb8x0 = self.up8(rdb8x0)
        rdb8x0 = self.conv_rdb8x0(rdb8x0)

        # 32x branch starts
        low32x_beforeRDB = self.conv32x(self.downshuffle(low2x, 16))
        rdb3 = self.RDB3(low32x_beforeRDB)
        rdb4 = self.RDB4(rdb3)
        rdb8x1 = torch.cat((rdb3, rdb4), dim=1)
        rdb8x1 = self.rdball1(rdb8x1) + low32x_beforeRDB
        ...
        rdb5 = self.RDB5(low16x_beforeRDB)
        rdb6 = self.RDB6(rdb5)
        rdb8x2 = torch.cat((rdb5, rdb6), dim=1)
        rdb8x2 = self.rdball2(rdb8x2) + low16x_beforeRDB
        rdb8x2 = self.up2(rdb8x2)
        rdb8x2 = self.conv_rdb8x2(rdb8x2)
        
        
        # rdb8x = torch.cat((rdb8x2, rdb8x1), dim=1)
        # rdb8x = self.conv_rdb8x3(rdb8x)

        rdb8x = torch.cat((rdb8x0, rdb8x1, rdb8x2), dim=1)
        rdb8x = self.conv_rdb8x4(rdb8x)
